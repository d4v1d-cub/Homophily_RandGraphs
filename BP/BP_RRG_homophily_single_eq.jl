...
function set_messages_ord(G::Int64)
    messages=zeros(2^G)
    messages[1] = 1
...
function run_BP_find_transition(c, seed, string1, string_init, max_iter, delta, G, 
    ...
    for alpha in alphas_list
        ...
        eq_e = ener(G, spins_combs, messages, alpha, temp, c)
        eq_m_norm, mag_components = mag_norm(G,spins_combs,messages,c, alpha, temp)
        ...
        write(w, string(temp) * "\t" * string(alpha) * "\t" * string(eq_e) * "\t" *string(eq_m_norm) 
                 * "\t" *string(eq_m_oriented) * "\t" * string(conv) * "\n")
